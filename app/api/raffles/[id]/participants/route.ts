...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!allowed) {
    ...
    return Response.json({ error: 'Demasiados pedidos de eliminação.' }, { status: 429 })
  }
  ...
  if (!raffleId || !uuidRe.test(raffleId)) {
    return Response.json({ error: 'ID do sorteio inválido.' }, { status: 400 })
  }
  if (!participantId || !uuidRe.test(participantId)) {
    return Response.json({ error: 'ID do participante inválido.' }, { status: 400 })
  }
  ...
  if (lookupError || !participant) {
    return Response.json({ error: 'Participante não encontrado neste sorteio.' }, { status: 404 })
  }
  ...
  if (error) {
    ...
    return Response.json({ error: 'Erro ao eliminar dados do participante.' }, { status: 500 })
  }
